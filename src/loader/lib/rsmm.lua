...
-- out of this chunk needs — it owns private F helpers and installs its own
...
local function _submodule_fn(name, env)
    ...
    local ran, err = pcall(m, env)
    if not ran then
        R.log("[rsmm] submodule rsmm." .. name .. " failed to install: " .. tostring(err))
        return false
    end
    return true
end
