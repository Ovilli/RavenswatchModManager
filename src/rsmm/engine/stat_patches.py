"""
Stat-patch primitives: schema definitions + byte-patcher for the cooked
numeric classes Ravenswatch ships.

Library used by `rsmm.cli.merge` (and indirectly by `rsmm.sdk` validation).
Not user-facing; mod authors call the SDK (`m.stat(...)`).

Supported schemas (see docs/INTERNALS.md):

    oCGlobalEntityValueSettings (body size = 23)
        Single float at body+0x0c. 143 game-balance constants.

    GameModifierDefinition (body size = 29)
        Single float at body+0x15. 19 modifier deltas.

    oCDtEnemyCampDifficultyDefinition (body size = 18)
