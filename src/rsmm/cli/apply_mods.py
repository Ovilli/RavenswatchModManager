...
def plan_apply(mods: list[Mod],
    ...
    """Compute (additions, removals, registrations, synthesized) given current
    state and on-disk mods.
    ...
    synthesized  : encoded paths that reached tier 3 — the game ships NOTHING
                   there, so no original can exist and nothing at that path may
                   ever be backed up as one. Includes new resource caches,
                   which are excluded from `registrations` for a different
                   reason (convention-loaded) but are equally mod-added.
    """
            ...
            if not enc:
                ...
                if not enc:
                    print(f"  [warn] {m.id}: no asset_map entry for '{decoded}' "
                          ...
                          file=sys.stderr)
                    continue
                # Reached tier 3, so `dec2enc` and `resolve_special` both
                # declined it: the game ships nothing at this path.
                synthesized.add(enc)
                if decoded.endswith(rsc_cache.CACHE_SUFFIX):
    ...
    for enc, (src, mod_id) in wanted.items():
        ...
        # A mod-ADDED path recorded with an original is a broken state entry,
        # not an applied file: `restore` would resurrect the generated bytes
        # from its .rsmm.bak instead of deleting them. Skipping here is what
        # made that permanent — the file matches its source, so it is never
        # re-applied and `apply_one` never gets the chance to heal it. Yield
        # the skip for exactly this case and let `apply_one`'s `mod_added`
        # branch clear the entry (measured 2026-09-07 on the shrine's level).
        broken_origin = (enc in synthesized and cur is not None
                         and (cur.get("orig_sha256") or cur.get("orig_sha1", "")))
        if (cur and cur.get("src_sha256") == src_sha and dest.exists()
                and sha256(dest) == src_sha and not broken_origin):
...
def apply_one(enc: str, src: Path, dest: Path, mod_id: str,
              ...
              journal: Journal | None = None, stage=None,
              mod_added: bool = False) -> None:
    ...
    if dest.exists():
        bak = dest.parent / (dest.name + BACKUP_SUFFIX)
        if mod_added:
            # FIRST, because this is the strongest evidence available: the path
            # reached tier 3 in `plan_apply`, so `dec2enc` AND `resolve_special`
            # both declined it and the game ships nothing here. Whatever sits at
            # this path is output from an earlier apply, and there is no
            # original to preserve.
            #
            # The `cur is not None` branches below cannot cover this, and the
            # gap is reachable: `restore --all` CLEARS the state, so the next
            # apply meets a leftover generated file with `cur is None` and used
            # to back it up as if it were vanilla. Measured 2026-09-07 on the
            # shrine's mod-owned level — its `.rsmm.bak` held an OLDER generated
            # level (31 placements, 30 grass + BonFire), and the resulting
            # non-empty `orig_sha256` meant `restore --all` would RESURRECT that
            # file instead of deleting it, leaving mod content in the game after
            # the manager was removed.
            #
            # Ordering it first also self-heals an entry already corrupted that
            # way; the warn-only branch below had to hedge with
            # `is_vanilla_encoded`, which cannot see the 575 shipped caches or
            # the sound banks. Those never reach here — they resolve at tier 1
            # or 2 and never enter `synthesized`.
            orig_sha = ""
            if bak.exists() and not dry_run:
                bak.unlink()
        elif (cur is not None and (cur.get("orig_sha256") or cur.get("orig_sha1", ""))
...
def restore_one(enc: str, cooking: Path, game_dir: Path,
    ...
    # WHO DECIDES: `orig_sha`, not the presence of a .rsmm.bak.
    #
    # A mod-ADDED file can still have a backup — the apply transaction snapshots
    # `dest` before overwriting it so a failed batch can roll back — and that
    # snapshot holds GENERATED bytes, not vanilla ones. Restoring it put the
    # mod's own file back and kept it installed: measured 2026-09-07, the
    # shrine's mod-owned level survived `restore --all` with the state
    # correctly recording `orig_sha256 == ""`. Uninstalling the manager is
    # supposed to leave the game as it was found, and it did not.
    #
    # `is_vanilla_encoded` still guards the other direction (a state that lost
    # its `orig_sha` for a file the game DOES ship must never be dropped), so
...
def cmd_apply(args, repo: Path, cooking: Path, game_dir: Path) -> int:
    ...
    additions, removals, registrations, synthesized = plan_apply(
