...
# --- a mod-ADDED path can never acquire an "original" ----------------------

def _apply_existing(tmp_path, state, *, mod_added, prior_bytes=b"older output"):
    """Apply over a path that already holds bytes from an earlier apply."""
    src = tmp_path / "src.bin"
    src.write_bytes(b"new output")
    dest = tmp_path / "cooking" / "generated.bin"
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_bytes(prior_bytes)
    AM.apply_one("Synth\\generated.bin", src, dest, "SomeMod", state,
                 dry_run=False, mod_added=mod_added)
    return dest, dest.parent / (dest.name + AM.BACKUP_SUFFIX)


def test_mod_added_path_is_never_backed_up_over_itself(tmp_path, state):
    """The gap that cost the shrine its uninstall, measured 2026-09-07.

    `restore --all` CLEARS the state, so the next apply meets a leftover
    generated file with no state entry. The pre-existing guard only fires when
    the state remembers the file (`cur is not None`), so this path fell through
    to the plain backup branch and recorded a generated level as the vanilla
    original. `restore --all` would then RESURRECT it — leaving mod content in
    the game after the manager was removed.
    """
    dest, bak = _apply_existing(tmp_path, state, mod_added=True)
    assert dest.read_bytes() == b"new output"
    assert not bak.exists(), "a mod-added path has no original to back up"
    assert state.active["Synth\\generated.bin"]["orig_sha256"] == "", \
        "empty orig_sha256 is what marks a file as deletable on restore"
...
def test_restore_drops_a_mod_added_file_even_when_a_backup_exists(tmp_path, state):
    """`restore --all` must leave the game as it was found.

    The apply transaction snapshots `dest` before overwriting so a failed batch
    can roll back, so a mod-ADDED file can legitimately have a `.rsmm.bak` full
    of GENERATED bytes. `restore_one` used to check `bak.exists()` before it
    looked at `orig_sha`, so it copied those bytes back and kept the file
    installed — measured 2026-09-07 on the shrine's mod-owned level, which
    survived `restore --all` with a correct `orig_sha256 == ""`.
    """
    enc = "Synth\\added.bin"
    dest = AM.encoded_to_dest(enc, tmp_path / "cooking", tmp_path)
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_bytes(b"generated")
    bak = dest.parent / (dest.name + AM.BACKUP_SUFFIX)
    bak.write_bytes(b"older generated")          # a rollback copy, not vanilla
    state.active[enc] = {"mod": "SomeMod", "src_sha256": "x", "orig_sha256": ""}

    assert AM.restore_one(enc, tmp_path / "cooking", tmp_path, state,
                          dry_run=False)
    assert not dest.exists(), "a mod-added file must be removed, not restored"
    assert not bak.exists(), "its rollback copy goes with it"
    assert enc not in state.active


def test_restore_still_restores_a_real_override(tmp_path, state):
    """The direction that must not regress: a file with a recorded original is
    put back from its backup, never deleted."""
    enc = "Synth\\overridden.bin"
    dest = AM.encoded_to_dest(enc, tmp_path / "cooking", tmp_path)
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_bytes(b"mod bytes")
    bak = dest.parent / (dest.name + AM.BACKUP_SUFFIX)
    bak.write_bytes(b"vanilla bytes")
    state.active[enc] = {"mod": "SomeMod", "src_sha256": "x",
                         "orig_sha256": "deadbeef"}

    assert AM.restore_one(enc, tmp_path / "cooking", tmp_path, state,
                          dry_run=False)
    assert dest.read_bytes() == b"vanilla bytes"
