...
<Tabs syncKey="os">
  ...
  <TabItem label="Linux" icon="linux">
    ...
    <Aside type="tip">
    **Prefer the AppImage if you want automatic updates.** The in-app updater
    replaces the running file, which it can only do for an AppImage you own —
    put it somewhere like `~/Applications`. Installed from the `.deb` or from a
    root-owned directory instead? The app offers a one-click **Install to
    ~/Applications** the next time an update lands, and updates itself from
    then on. The old system copy stays put; the app tells you the
    `sudo apt remove` line for it.
    </Aside>
...
From a source checkout, `install-loader` downloads the signed prebuilt loader the
first time, so you do not need a C++ toolchain. Only loader developers build it
themselves: `src/loader/build.sh` (Linux cross-compile, MinGW) or
`src\loader\build.bat` (Windows); both need CMake.
