---
title: Spawning
description: How the engine creates a single entity at a position — the spawn primitive.
---

:::note
Status: confirmed via live Ghidra MCP session against
`Ravenswatch.exe`. Companion to
[`MOD_HOOKS.md`](/reverse-engineering/mod-hooks/) (level-load pipeline + library ABI)
and [`HOOKPOINTS.md`](/reverse-engineering/hookpoints/) (feature → function map).
:::
...
[`MOD_HOOKS.md`](/reverse-engineering/mod-hooks/#registry-key-is-32-bytes-not-4)).
...
[`MOD_HOOKS.md` "The mod hook surface"](/reverse-engineering/mod-hooks/#the-mod-hook-surface):
...
* **Listener-based content** — a mod can subscribe to
  ...
  [`MOD_HOOKS.md` "Still to be confirmed"](/reverse-engineering/mod-hooks/#still-to-be-confirmed))
...
* `oCGameNamedEvent` listener registration API (open across
  [`MOD_HOOKS.md` "Still to be confirmed"](/reverse-engineering/mod-hooks/#still-to-be-confirmed)
  and [`HOOKPOINTS.md` recommended first POC](/reverse-engineering/hookpoints/#recommended-first-poc-for-rsmmhook-validation-task-6)).
